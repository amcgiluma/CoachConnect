...
function Integrations() {
  type IntegrationStatus = { stripe: boolean; stripe_status?: 'not_connected' | 'pending' | 'active' | 'unavailable'; stripe_requirements_due?: number; providers: string[]; custom_video_url?: string }
  const [status, setStatus] = useState<IntegrationStatus>({ stripe: false, stripe_status: 'not_connected', providers: [] })
  useEffect(() => { api<IntegrationStatus>('/api/v1/coach/integrations').then(setStatus).catch(() => undefined) }, [])
