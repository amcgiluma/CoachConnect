...
import { ArrowLeft, ArrowRight, BadgeCheck, Bell, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Clock3, CreditCard, Eye, FileCheck2, Globe2, Languages, LayoutDashboard, LoaderCircle, LogOut, MapPin, MessageCircle, Paperclip, Pencil, Plus, Send, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Star, Upload, UserRound, Video, X, Zap } from 'lucide-react'
...
function LocationQuestion({ title, initialValue, onSelect }: { title: string; initialValue?: string; onSelect: (value: string) => void }) {
  ...
  return (
    <div className="question-content location-question">
      <p className="eyebrow">Elige una ubicación real</p>
      <h2 id="question-title">{title}</h2>
      <p className="location-copy" id="location-help">
        Empieza a escribir y selecciona la coincidencia correcta.
      </p>
      <form className="location-form" onSubmit={submit}>
        <label className="sr-only" htmlFor="location-input">
          Ciudad, municipio, barrio o código postal
        </label>
        <div className={`location-input-shell ${selectedLocation ? 'is-selected' : ''}`}>
          ...
          <input
            ...
            placeholder="Por ejemplo, Sevilla, Getxo o 29010"
            autoComplete="off"
            aria-describedby="location-help location-status"
            autoFocus
          />
          <button type="submit" disabled={!selectedLocation} aria-label="Continuar con esta ubicación">
            {selectedLocation ? <Check aria-hidden="true" /> : <ArrowRight aria-hidden="true" />}
          </button>
        </div>
        <div className="location-status" id="location-status" aria-live="polite">
          {lookupState === 'loading' && (
            <span>
              <LoaderCircle className="spin" aria-hidden="true" /> Buscando coincidencias…
            </span>
          )}
          ...
          {selectedLocation && (
            <span className="location-confirmed">
              <Check aria-hidden="true" /> Ubicación seleccionada
            </span>
          )}
        </div>
        <div className="location-suggestions" id="location-suggestions" role="listbox" aria-label="Coincidencias de ubicación">
          {suggestions.map((suggestion) => (
            <button type="button" role="option" aria-selected="false" key={suggestion.id} onClick={() => chooseLocation(suggestion)}>
              <MapPin aria-hidden="true" />
              <span>
      ...
      <button type="button" className="location-online" onClick={() => onSelect('Cualquier lugar si es online')}>
        <span className="option-icon">
          <Globe2 aria-hidden="true" />
        </span>
        <span>
          <strong>Cualquier lugar si es online</strong>
          <small>Buscaremos entrenadores online sin filtrar por zona.</small>
        </span>
        <ArrowRight aria-hidden="true" />
      </button>
    </div>
  ...
  return (
    <section className="account-screen">
      ...
      <div className="account-tabs">
        <Link className="active" to="/cuenta">
          Reservas
        </Link>
        <Link to="/mensajes">Mensajes</Link>
        {profile?.role === 'coach' && <Link to="/profesional?tab=reviews">Valoraciones</Link>}
      </div>
...
function Messages({ onAuth }: { onAuth: () => void }) {
  ...
  return (
    <section className="messages-screen">
      <Link className="back-link messages-back" to="/cuenta">
          ...
          {conversations.map((item) => {
            ...
            return (
              <button className={active === item.id ? 'active' : ''} key={item.id} onClick={() => chooseConversation(item.id)}>
                ...
                <span>
                  <strong>{other?.display_name || 'CoachConnect'}</strong>
                  <small>Conversación segura</small>
                </span>
              </button>
            )
          })}
        ...
        <div className="conversation">
          {active && (
            <div className="conversation-actions">
              <strong>{activeOther?.display_name || 'CoachConnect'}</strong>
              <button className="text-button" onClick={reportConversation}>
                Denunciar
              </button>
              <button className="text-button" onClick={blockActiveUser}>
                Bloquear
              </button>
            </div>
          )}
            ...
            {messages.map((item) => (
              <div className={`message ${item.sender_id === user.id ? 'outgoing' : 'incoming'} ${item.delivery_status === 'sending' ? 'sending' : ''}`} key={item.id}>
                ...
                {item.attachment_path && (
                  <button className="attachment-link" onClick={() => openAttachment(item.attachment_path || '')}>
                    <Paperclip /> Abrir archivo
                  </button>
                )}
                <small>
                  {item.delivery_status === 'sending'
          ...
          {active && (
            <form className="chat-composer" onSubmit={send}>
              <label className="attachment-button" aria-label="Adjuntar archivo">
                <Paperclip />
                <input type="file" accept=".pdf,image/jpeg,image/png,image/webp" onChange={attach} />
              </label>
              <input aria-label="Mensaje" value={body} onChange={(event) => setBody(event.target.value)} placeholder="Escribe un mensaje…" disabled={busy} />
              <button type="submit" aria-label="Enviar" disabled={busy || !body.trim()}>
                {busy ? <LoaderCircle className="spin" /> : <Send />}
              </button>
            </form>
...
function ProPortal({ onAuth }: { onAuth: () => void }) {
  ...
  return (
    <section className="pro-screen">
      <div className="pro-header">
        <div>
          <p className="eyebrow">CoachConnect para profesionales</p>
          <h1>
            Tu trabajo.
            <br />
            <em>Bien visible.</em>
        ...
        <div className="pro-header-actions">
          {profile?.role === 'coach' && (
            <Link className="preview-link" to={`/entrenadores/${user.id}?preview=1`}>
      ...
      <div className="pro-shell">
        <aside className="pro-sidebar">
          <div className="pro-user">
            <div className="avatar">{(profile?.display_name || user.email || 'CC').slice(0, 2).toUpperCase()}</div>
            <div>
              <strong>{profile?.display_name || user.email}</strong>
              <span>{profile?.role === 'coach' ? 'Perfil profesional' : 'Completa tu alta'}</span>
            </div>
          </div>
          {[
            ['overview', 'Resumen'],
        ...
        <div className="pro-content">
          {tab === 'overview' && <ProOverview profile={profile} userId={user.id} onTab={setTab} />}
          {tab === 'requests' && <BookingRequestsPanel />}
          {tab === 'reviews' && <CoachReviewsPanel />}
          {tab === 'profile' && <CoachOnboarding onSaved={() => api<Profile>('/api/v1/me').then(setProfile)} />}
          {tab === 'services' && <ServicesForm />}
          {tab === 'availability' && <AvailabilityForm />}
          {tab === 'validation' && <CredentialForm userId={user.id} />}
...
function ProOverview({ profile, userId, onTab }: { profile: Profile | null; userId: string; onTab: (tab: string) => void }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      >('/api/v1/coach/profile').catch(() => null),
...
function CoachOnboarding({ onSaved }: { onSaved: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      languages: String(values.languages_text || 'es')
        .split(',')
        .map((item) => item.trim().toLowerCase())
        .filter(Boolean),
    }
    delete (payload as Record<string, unknown>).languages_text
    try {
      await api('/api/v1/coach/onboarding', {
        method: 'POST',
        body: JSON.stringify(payload),
      })
  ...
  return (
    <ProForm title="Perfil profesional" intro="Lo esencial para que un consumidor entienda en segundos si encajas." onSubmit={submit}>
      <label>
        Nombre visible
        <input name="display_name" required minLength={2} defaultValue={current?.profiles?.display_name || ''} />
      </label>
      <label>
        Titular profesional
        <input name="headline" required minLength={5} placeholder="Fuerza y movilidad sin complicaciones" defaultValue={current?.headline || ''} />
      </label>
      <label className="wide">
        Sobre tu método
        <textarea name="bio" required minLength={20} rows={5} defaultValue={current?.bio || ''} />
      </label>
      <label>
        Ciudad
      ...
      <label>
        Modalidad
        <select name="mode" defaultValue={current?.mode || 'hibrido'}>
          <option value="hibrido">Online y presencial</option>
      ...
      <label>
        Años de experiencia
        <input name="years_experience" type="number" min="0" defaultValue={current?.years_experience || 0} />
      </label>
      <label>
        Idiomas
        <input name="languages_text" defaultValue={(current?.languages || ['es']).join(', ')} placeholder="es, en" />
      </label>
      <Button type="submit" disabled={busy}>
        {busy && <LoaderCircle className="spin" />} Guardar perfil
      </Button>
    </ProForm>
  )
}
...
function AvailabilityForm() {
  ...
  useEffect(() => {
    ...
    api<{ bookings: BookingRecord[]; exceptions: any[] }>(`/api/v1/coach/calendar?from=${encodeURIComponent(weekStart.toISOString())}&to=${encodeURIComponent(weekEnd.toISOString())}`)
      .then((data) => {
        ...
        setCalendarExceptions(data.exceptions)
        setCalendarError('')
      })
  ...
  const save = async () => {
    const rules = active.map((weekday) => ({
      weekday,
      starts_at: startsAt,
      ends_at: endsAt,
    ...
    try {
      await api('/api/v1/coach/availability', {
        method: 'PUT',
  ...
  const addException = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(formElement)
    try {
      const item = await api<any>('/api/v1/coach/availability/exceptions', {
        method: 'POST',
        body: JSON.stringify({
      ...
      setCalendarExceptions((current) => [...current, item])
      formElement.reset()
  ...
  const removeException = async (id: string) => {
    try {
      await api(`/api/v1/coach/availability/exceptions/${id}`, {
        method: 'DELETE',
      })
      setExceptions((current) => current.filter((item) => item.id !== id))
      setCalendarExceptions((current) => current.filter((item) => item.id !== id))
      toast.success('Bloqueo eliminado')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'No se pudo eliminar')
    }
  }
  const toggleRespondsNow = async () => {
    const enabled = !respondsNow
    try {
      await api('/api/v1/coach/responds-now', {
        method: 'PATCH',
        body: JSON.stringify({ enabled }),
      })
      setRespondsNow(enabled)
    } catch (error) {
  ...
  return (
    <section className="agenda-manager">
      <div className="section-title">
        <div>
          <p className="eyebrow">Agenda profesional</p>
          <h2 className="form-title">Tus sesiones, claras.</h2>
        </div>
        <button className={`status-toggle ${respondsNow ? 'active' : ''}`} onClick={toggleRespondsNow}>
          <span className="live-dot" /> {respondsNow ? 'Respondo ahora' : 'Activar Responde ahora'}
        </button>
      </div>
      <p className="form-intro">Consulta qué sesiones están reservadas y administra debajo tu disponibilidad recurrente.</p>
      <section className="week-calendar" aria-label="Calendario semanal de sesiones">
        <header>
          <div>
          ...
          <div className="calendar-controls">
            <button aria-label="Semana anterior" onClick={() => setWeekStart((current) => addDays(current, -7))}>
              <ChevronLeft />
            </button>
            <button onClick={() => setWeekStart(startOfWeek(new Date()))}>Hoy</button>
            <button aria-label="Semana siguiente" onClick={() => setWeekStart((current) => addDays(current, 7))}>
              <ChevronRight />
            </button>
          </div>
        </header>
        ...
        {calendarLoading ? (
          <LoadingBlock label="Cargando la semana" />
        ) : (
          <div className="week-grid">
            {weekDays.map((date, index) => {
              ...
              return (
                <article className={today ? 'today' : ''} key={date.toISOString()}>
                  ...
                  <div className="week-events">
                    {dayBookings.map((item) => (
                      <div className={`calendar-event status-${item.status.replaceAll('_', '-')}`} key={item.id}>
                        <time>{new Date(item.starts_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</time>
                        <strong>{item.coach_services?.name || 'Sesión'}</strong>
                        <span>{item.profiles?.display_name || 'Cliente'}</span>
                        <small>{statusLabel[item.status] || item.status}</small>
                      </div>
                    ))}
                    {dayBlocks.map((item) => (
                      <div className="calendar-event blocked" key={item.id}>
                        <time>{new Date(item.starts_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</time>
                        <strong>{item.label || 'Bloqueo'}</strong>
                        <small>No disponible</small>
                      </div>
                    ))}
                    {!dayBookings.length && !dayBlocks.length && <span className="calendar-empty">Sin sesiones</span>}
                  </div>
                </article>
              )
            })}
      ...
      <section className="availability-settings">
        <div>
          <p className="eyebrow">Horario recurrente</p>
          <h3>Cuándo pueden reservarte</h3>
          <p>Activa tus días habituales y define una franja. Los bloqueos siempre tienen prioridad.</p>
        </div>
        ...
        <div className="day-grid">
          {days.map((day, index) => (
            <button type="button" className={active.includes(index) ? 'active' : ''} onClick={() => setActive((items) => (items.includes(index) ? items.filter((item) => item !== index) : [...items, index]))} key={day}>
      ...
      <form className="exception-form" onSubmit={addException}>
        ...
        <label>
          Motivo
          <input name="label" placeholder="Vacaciones" />
        </label>
        <Button type="submit">Añadir bloqueo</Button>
      </form>
      <div className="compact-list exception-list">
        {exceptions.map((item) => (
          <div key={item.id}>
            <div>
              <strong>{item.label || 'Bloqueo'}</strong>
              <span>
                {new Date(item.starts_at).toLocaleString('es-ES')} — {new Date(item.ends_at).toLocaleString('es-ES')}
              </span>
            </div>
            <button className="danger-action" onClick={() => removeException(item.id)}>
              Eliminar
            </button>
          </div>
        ))}
